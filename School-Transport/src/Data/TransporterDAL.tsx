...
export const AddTransporterToDatabase = async (
  ...
  try {
    await setDoc(
      doc(FIRESTORE_DB, DATABSE, "Users", GetUserUid(), "PersonalDetails"),
      data
    ).then(() => {
      response = "Success";
    });
...
export const CreateParentVerification = async (
  firstName: string,
  lastName: string,
  email: string,
  cellphone: string,
  ...
  const parentVerificationRef = doc(
    FIRESTORE_DB,
    DATABSE,
    "Verification",
    "Parents",
...
export const CheckIfParentVerificationExists = async (
  verificationCode: string
) => {
  const parentColectionRef = collection(
    FIRESTORE_DB,
    DATABSE,
    "Verification",
    "Parents"
...
export const AddBusinessDetails = async (businessDetails: BusinessDetails) => {
  ...
  try {
    await setDoc(
      doc(FIRESTORE_DB, DATABSE, "Users", GetUserUid(), "BusinessDetails"),
      data
    ).then(() => {
      response = "Success";
    });
